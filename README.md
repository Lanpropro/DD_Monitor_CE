...
- ⏺️ 每格独立录制直播流、保存最近数分钟的即时回放（需 FFmpeg）
- 🎧 每一路可以单独路由到左声道 / 右声道
- 🛠️ 支持断流重连和画面卡死检测
- 🧩 支持插件（`plugins_user/<名字>/plugin.py`）
- 💾 自动保存布局（分横竖屏各记一套）、音量、画质和界面状态

录制在画面格子底栏点击 `●` 开始/停止；右键格子可以开启即时回放缓存，
或在录制期间保存最近一段。默认 MP4 + 原始流直存，不重编码；只有选择 H.264
重编码时，设置里的码率和帧率才生效。文件只含这一路直播的画面和原始声音，
不含软件弹幕或控件。格子被布局隐藏、主播下播或关闭这一路时会自动停止。
建议在「设置 → 录制」为满盘备用目录选择**另一块磁盘**；同一磁盘无法在空间耗尽后续录。
主磁盘腾出足够空间后，完整录制会优先导回主目录；否则保存在备用目录并提示。
...
| 录制组件 | FFmpeg（只有使用录制或即时回放时需要；可在「设置 → 录制」选择 `ffmpeg.exe`） |
...
录制功能还需自行安装 FFmpeg 并把它加入 `PATH`，或在「设置 → 录制」指定 `ffmpeg.exe`；
目前发布包不内置 FFmpeg，普通观看不受影响。
