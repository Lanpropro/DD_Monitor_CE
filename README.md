...
鼠标停在关注列表里**正在直播**的主播上 **1 秒**，旁边会弹出一个小画面（不抢焦点、**静音**、低画质、**圆角**）：

- 小窗里**只有画面**：没有标题栏、没有文字提示，整块都是视频（状态只写进运行日志，方便排查）
- 不到 1 秒就离开不会弹；鼠标一移开就收掉（约 0.1 秒），播放器同时释放，不会一直占带宽和 CPU
- 直接移到另一个主播身上时，旧画面立刻收掉，而不是等 1 秒后才换
- 只有开播的条目才预览；未开播的停多久都不会弹
- 小窗跟条目对齐，并**往左压住侧栏约 1/3 宽度**（离鼠标近一点）；贴到屏幕边缘会自动往里收
- 不想要的话：设置 →「常规」里关掉「鼠标停在关注列表的直播上 2 秒，弹出小画面预览」
- 实现放在 `ddm/preview.py`（`StreamPreview` 小窗 + `HoverPreview` 悬停计时），复用了画面墙那套取流与 VLC 播放封装
...
- `dev\run-checks.cmd`：跑一遍全部自检（当前 15 个，全部通过）
