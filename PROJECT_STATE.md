...
- 录制占用二次整改（用户报「单开一格录制占用也有点高，影响打游戏」，实测下来
  ...
  10) **录制功能总开关**（用户又追加）：`recording_enabled`，设置页「录制 →
     录制功能」的勾。关掉时每格底栏收起「● 录制」按钮、右键菜单里没有录制项、
     正在录制的会话**正常收尾并导出**（已录到的部分不丢，不是丢弃）、录制参数
     一起置灰；即时回放那一组不受连累。按钮可见性统一在
     `Tile._update_recording_button()` 里算 —— 录制状态每次刷新都会走它，
     否则各处刷新会把收起来的按钮又露出来。
  `selfcheck_replay_scope.py` 从 7 节扩到 10 节（新增上限三连 + Job/重连 +
  裁剪节流 + 两个总开关）。
  没做的：让 VLC 复用同一路流做录制（`sout` / `#duplicate`）能省掉全部额外带宽，
  但要重做「最近 N 分钟」的分段架构、开关录制得重启那一格，收益只剩修完并发之后
  的最后一半，先不动。
- **退出全屏卡一下**（用户报，附 4K60 录屏）：`freezedetect` 量出来是
  1.717 s → 2.000 s **完全冻结 283 ms** 再突变回窗口。分头实测（4K，dpr 1.5）：
  `grabWindow(0)` 抓全屏 40~53 ms、建遮盖窗 + `setPixmap`(4K) 42.7 ms，剩下
  约 190 ms 是窗口样式切换 + 9 个格子的 VLC 原生窗口重配。**这段卡顿本质是
  「遮盖图忠实地盖住了一个慢流程」** —— 流程 283 ms 里主线程全程僵着，遮盖图
  既不淡也不动，于是看起来就是卡死再一跳。三处改动：
  1) 遮盖底图**先缩到一半**再 `setPixmap`（4K 下 42.7 → 16 ms，
     `setScaledContents` 会拉回去，过渡的百来毫秒里看不出模糊）；
  2) 遮盖不再「到点直接关」，改成 120 ms 后**淡出** 160 ms —— 期间下面的格子
     本来就在播，画面是「逐渐透出来」而不是「跳一下」；
  3) `WallPanel.set_fullscreen_tile(stagger=, first=)` 只把布局算好，其余格子
     交给 `reveal_tiles_staggered()` **一帧两格**地露面，刚退出来那格立刻显示。
     单格 VLC 窗口重配约 23 ms，一批两格约 46 ms。拆帧留下的间隔是给淡出动画
     推进用的 —— 连着做完主线程一样是僵的，那就白拆了。
  另外进出全屏各打一行 `[全屏] … （遮盖 / 切换 / 布局）`，下次再觉得卡不用
  从头猜。新增 `selfcheck_fullscreen_switch.py`（6 节）。
- **关注栏头像时有时无**（用户报「包括收缩后的关注栏」）：根因是头像的可见性
  **散在三处、逻辑还不一致** —— `_render_face()` 末尾写「视频没在播就露脸」、
  `set_thumb_size()`（收起 / 展开切换）里写「有 pixmap 才露脸」、初始化时写死
  `False`。谁最后跑谁说了算，同一条目会随布局变化在「露脸」和「不露」之间跳；
  收起的关注栏**整条只剩一个头像**（封面本来就不显示），走了后一条就整个空掉。
  另外 `_render_face()` 拿不到头像时**直接 return**，连尺寸和位置都不更新，
  收起后的留白也全是错的。三处修法：
  1) 可见性统一到 `_render_face()` 里算（三个调用点自动受益）：紧凑条目一定要
     露脸，卡片模式头像还没下到就先不占那一块，视频预览占着整块时都让位；
  2) 没图时不再空着，用主播名首字顶着（取不到就「?」）；
  3) 头像也按房间号留档：`images.py` 加 `room_avatar_path` / `load_room_avatar` /
     `remember_room_avatar` + `CachedAvatarLoader`，下载完顺手留一份；
     `app.py` 启动 120 ms 时 `load_cached_avatars()` 先用本地那张顶上去 ——
     真实头像要等状态轮询（默认 1.2 秒）拿到 `face` 才开始下，那段空窗期正是
     用户看到空白的时候。
  新增 `selfcheck_sidebar_avatar.py`（7 节：收起时的占位、两入口一致、有图、
  卡片模式、来回切换不抖、留档路径、无名字兜底）。
  注：另一个会话（Codex）先前试过同一处（`bce6cba`）又 `6762775` 撤掉了 ——
  它把 `face` 一律 `setVisible(True)`，卡片模式下会盖住封面，所以失败。
- 自检现状（本次实测重跑）：全量 **48 项**。
  `selfcheck_recording.py` 之前一直失败，而且**与录制占用那轮改动无关** ——
  把改动 `git stash` 回基线跑，失败点一模一样。
